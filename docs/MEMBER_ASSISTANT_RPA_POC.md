...
截至 2026-05-17 的真实桌面验证结果：

```text
[x] 多群发送计划能正确生成目标 chat_name 和回复内容
[!] AppleScript --run 未稳定定位到 汽车贷款小助手，不能判定为可靠发送通道
[x] Computer Use 可以手动定位 汽车贷款小助手 并把第一条任务写入正确输入框
[x] 本次验证未发送消息，草稿验证后已清空
```

因此，当前发送侧的结论是：发送任务映射成立，Computer Use 人机协同发送可行；纯 AppleScript 自动定位群聊仍需替换或增强，不能直接作为多群无人值守发送通道。

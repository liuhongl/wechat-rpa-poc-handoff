...
[!] 连续扫描进程内会按 event_id 去重，避免同一条可见消息多轮重复生成计划
[!] AX event_id 会对同一可见快照内相同“群 + 发送人 + 内容”的重复 @ 增加 occurrence 序号，避免两条相同消息被合并成一条
[!] occurrence 仍不是企业微信服务端 msgid，生产级还需要继续引入更稳定的消息身份来源
[!] 当前不会把 dry-run 规划结果写入永久 processed state，真实持久去重应放在写草稿/发送成功之后
```
...
[x] 只消费目录里尚未处理过的新快照
[x] 传入 --snapshot-cursor-file 后，扫描器重启也会跳过已消费快照

...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Plan multi-group member-assistant reply jobs from simulated/decrypted WeCom message-audit records."
    )
    parser.add_argument(
        "--sample-plaintext-json",
        type=Path,
        default=ROOT_DIR / "tests" / "fixtures" / "multi_group_msgaudit_messages.json",
    )
    parser.add_argument(
        "--group-targets-json",
        type=Path,
    ...
    parser.add_argument(
        "--state-file",
        type=Path,
        default=ROOT_DIR / "data" / "member_assistant_poc" / "multi_group_processed_msgids.json",
    )
    parser.add_argument(
        "--out",
        type=Path,
    ...
    records = extract_plain_text_records(messages)
    ...
    if not jobs:
        print("[multi-group-reply] no pending reply jobs", flush=True)
